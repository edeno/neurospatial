...
def bin_spike_train(
    ...
    # For 1D positions, ensure we handle the shape correctly
    if positions.ndim == 1:
        positions = positions.reshape(-1, 1)

    n_dims = positions.shape[1]
    spike_positions = np.zeros((len(spike_times_valid), n_dims), dtype=np.float64)
    for d in range(n_dims):
        spike_positions[:, d] = np.interp(spike_times_valid, times, positions[:, d])

    # Map spike positions to bins
    spike_bins = env.bin_at(spike_positions)

    # Filter out invalid bins (positions outside environment)
    valid_bin_mask = spike_bins >= 0
    spike_bins_valid = spike_bins[valid_bin_mask]

    if len(spike_bins_valid) == 0:
        return spike_counts

    # Count spikes per bin
    spike_counts = np.bincount(spike_bins_valid, minlength=n_bins).astype(np.float64)
...
def bin_spike_trains(
    ...
    *,
    ...
    # Compute occupancy once (shared across all neurons)
