...
and other angular measurements.

Imports
-------
>>> from neurospatial.stats.circular import rayleigh_test
>>> from neurospatial.stats import circular_mean, mean_resultant_length
...
**Compute circular mean/variance?**
    Use ``circular_mean()``, ``circular_variance()``, ``mean_resultant_length()``.

**Wrap angles to a standard range?**
...
__all__ = [  # noqa: RUF022
    ...
    # Public circular statistics (formerly private in metrics/circular.py)
    "circular_mean",
    "circular_variance",
    "mean_resultant_length",
    # Angle utilities
    "wrap_angle",
    # GLM circular basis (from metrics/circular_basis.py)
    "CircularBasisResult",
    "circular_basis",
...
def _validate_paired_input(
    ...
    *,
    ...
    arr2 : ndarray of shape (n,)
        Second array of paired values, must match length of arr1.
    name1 : str
        Name of first array for error messages.
    name2 : str
        Name of second array for error messages.
    min_samples : int, default=3
        Minimum required number of valid samples after NaN removal.

    Returns
    -------
    arr1_valid : ndarray of shape (m,)
        Validated first array with NaN pairs removed, where m <= n.
    arr2_valid : ndarray of shape (m,)
        Validated second array with NaN pairs removed, where m <= n.

    Raises
    ------
    ValueError
        If lengths don't match or insufficient samples after NaN removal.
    """
...
def _validate_tuning_data(
    ...
    *,
    ...
    """
    Validate tuning curve data (bin centers and firing rates).
    ...
    require_variation : bool, default=True
        If True, require non-zero and non-constant firing rates.
        Set to False for plotting functions that can handle edge cases.

    Returns
    -------
    bin_centers : ndarray of shape (n_bins,)
        Validated bin centers as float64 array.
    firing_rates : ndarray of shape (n_bins,)
        Validated firing rates as float64 array.
    ...
    if len(bin_centers) != len(firing_rates):
        raise ValueError(
            ...
            f"Fix: Ensure both came from the same tuning curve computation."
        )
    ...
    # Check for negative firing rates
    ...
    if require_variation:
        # Check for all-zero firing rates
        if np.sum(firing_rates) == 0:
            raise ValueError(
                "All firing rates are zero. Cannot compute metrics.\n"
        ...
        if np.ptp(firing_rates) == 0:
            raise ValueError(
                "All firing rates are constant. Cannot compute metrics.\n"
...
# =============================================================================
# Public API - Circular Descriptive Statistics
# =============================================================================
...
# =============================================================================
# Data Classes - GLM Circular Basis
# =============================================================================
...
# =============================================================================
# Internal Helper Functions - GLM Circular Basis
# =============================================================================
...
# =============================================================================
# Public API - GLM Circular Basis Functions
# =============================================================================
