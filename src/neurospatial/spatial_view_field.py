"""Compute spatial view fields (firing rate by viewed location).

Spatial view fields represent neural activity as a function of *where the animal
is looking* rather than where the animal is located. This is the key distinction
from place fields: spatial view cells encode the location being viewed, not the
animal's position.

Common Use Cases
----------------
- Spatial view cell analysis (what location is being viewed)
- Comparing place fields vs view fields for cell classification
- Gaze-contingent neural analysis
- Primate hippocampal recordings

Which Function Should I Use?
----------------------------
**Computing view field from spikes?**
    Use ``compute_spatial_view_field()`` to compute firing rate as a function
...
**Different gaze models?**
    ...
    direction, or ``gaze_model="ray_cast"`` for boundary intersection.
...
2. Access the field and view occupancy::

    >>> field = result.field  # Firing rate per bin  # doctest: +SKIP
    >>> view_occ = result.view_occupancy  # Time viewing each bin  # doctest: +SKIP

3. Compare with place field::

    >>> from neurospatial import compute_place_field  # doctest: +SKIP
    >>> place_field = compute_place_field(env, spike_times, times, positions)
    >>> # View field and place field should differ for spatial view cells

Key Difference from Place Fields
--------------------------------
- **Place field**: Neural activity binned by *animal position*
- **View field**: Neural activity binned by *viewed location*

For place cells, these are similar. For spatial view cells, the view field
will show tuning to a specific viewed location regardless of where the animal
is positioned.

Coordinate Conventions
----------------------
**Gaze direction** (heading):
- 0 radians = East (standard mathematical convention)
- pi/2 radians = North

**Viewed location** depends on gaze model:
