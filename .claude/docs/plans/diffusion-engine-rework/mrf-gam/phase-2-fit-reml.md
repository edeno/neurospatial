...
- Wire `fit_mrf_gam`: if `penalty is None` → `select_penalty_by_reml` (or skip at `r==0`); else fixed λ, `reml_objective=None`, **record the supplied `penalty`**. Then the final `_newton_fit_numpy` at the chosen λ. Compute the floored `firing_rate = max(exp(η), _RATE_FLOOR)` on live bins and the deviance ([designs.md#deviance](designs.md#deviance)) from that stored rate.
