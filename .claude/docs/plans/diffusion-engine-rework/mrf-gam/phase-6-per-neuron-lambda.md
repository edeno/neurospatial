...
- Docs: `compute_spatial_rate` docstring + CHANGELOG — `pooled=False` for per-neuron smoothing; note the cost (REML per unit), fixed-penalty precedence, zero-spike fallback, shared-basis `r==0` behavior, and that a boundary warning means λ itself is weakly identified even when the fitted field is stable.
