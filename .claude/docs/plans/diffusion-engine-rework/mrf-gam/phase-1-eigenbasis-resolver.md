...
- **Pure selector (no eigensolve).** Add `select_live_basis(Q, Lam, mode_comp, volumes, labels, occupancy, *, rank) -> MRFBasis` to `ops/diffusion.py` (pure, array-only, unit-testable without an `Environment`), per [designs.md → pure selector](designs.md#resolver): live components (`Σ occupancy > 0`), `r_eff = max(n_live_components, min(n_live_bins, R))` (`R=250` when `None`). **Reserve one null per live component FIRST** (its smallest-λ mode), **then** fill the remaining `r_eff − n_live_components` slots globally by smallest λ — **not** a plain global "first `r_eff`", which can drop a whole live component when a small positive mode of one component sorts before another component's numerically-positive null (Finding 1). Then **apply `M^{-1/2}` and restrict to live bins** (`B = (Q / sqrt(volumes)[:,None])[live_bins]`), and zero the reserved nulls (`d[:n_live_components] = 0`). Degenerate `n_live_bins == 0` → `MRFBasis((0,0), (0,), empty, 0)`. `_mrf_basis`'s growth loop guarantees **every live component is represented** in the global basis before selection.
...
| `test_null_per_live_component_asymmetric` | **weak-link regression (Finding 1):** two live components of very different connectivity (A a tight clique with a small positive eigenmode ~1e-10, B a weak chain whose null is numerically positive and sorts *after* A's positive mode) at `rank == n_live_components`: assert **both** components appear in `B` (one null each; `mode_comp` of the reserved nulls covers every live component), where a plain global "first r_eff" would omit B entirely. |
