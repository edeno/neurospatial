...
- **Cached global resolver (owns the eigensolve).** Add `Environment._mrf_eigenbasis` as a `versioned_cached_property` returning a **mutable holder** `{"Q": (n_bins, G), "Lam": (G,), "mode_comp": (G,), "G": int}` (grow-by-replace, mirroring `_diffusion_eigenbasis` at `fields.py:535`; keyed by geometry only — no `(sigma, tol)`). Plus `_ensure_global_modes(holder, S, labels, needed_G)` ([designs.md → cached resolver](designs.md#resolver)): grows the holder to `≥ needed_G` global modes only when short, and **recovers each mode's source W-component** from its (component-local, exact-zero-off-component) eigenvector — no change to `_symmetric_eigenbasis`, and it replaces the previously-undefined `labels_of_modes`.
