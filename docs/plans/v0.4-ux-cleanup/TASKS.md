...
- [x] **1.3** **Polar env safety.** Add `is_polar: bool` property and `coordinate_kind: Literal["cartesian", "polar"]` attribute to `Environment`. `from_polar_egocentric` sets `coordinate_kind="polar"`. `Environment.distance_to`, `Environment.distance_between`, `Environment.bin_at` (when called with (x, y) shape input on a polar env), and the default `plot_field` axis labels raise `ValueError("This environment is polar; method X assumes Cartesian coordinates")`.
  ...
  Closes review §8.4.
  Acceptance: tests that construct a polar env and assert clear `ValueError` from each Cartesian-assuming method. _Landed in `7377271`. Added `Environment.coordinate_kind` (default `"cartesian"`), `is_polar` property, and `_check_cartesian` helper. `from_polar_egocentric` flips the flag. Wired into `bin_at`, `distance_between`, and `distance_to(metric="euclidean")`; `distance_to(metric="geodesic")`, `neighbors`, `path_between`, and `reachable_from` continue to work because they read only the connectivity graph. `plot_field` axis labels deferred to M5 (visualization cleanup). `EnvironmentProtocol` extended with `coordinate_kind`, `is_polar`, `_check_cartesian`. Persistence: `to_file`/`from_file` and `Environment.copy` carry `coordinate_kind` across; the default value is omitted from serialized JSON so older Cartesian envs round-trip unchanged. 7 regression tests cover the flag, the raises, and the round-trip. Drive-by fix: `tests/environment/test_visualization.py:330` was passing unsorted spike_times to `compute_spatial_rate`; M1 1.7's sortedness check rejected it; sorted in place._
