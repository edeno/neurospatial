...
- [x] **1.5** **Batch metric `nan` substitution.** [encoding/_metrics.py:548-550, 672-674](../../../src/neurospatial/encoding/_metrics.py) (`batch_grid_scores`, `batch_border_scores`) and [encoding/border.py:243-244](../../../src/neurospatial/encoding/border.py) (Dijkstra failure):
  ...
  Acceptance: feeding a deliberately-broken neuron (e.g., empty firing rate) produces a warning with count and the `failures` mask flags the right index. _Landed in `dea5404` (initial) and the M1 review-response commits (border_score raise-on-failure, legitimate-NaN distinction, NumPy-2 `__array__` semantics). New `BatchScoresResult` frozen dataclass carrying parallel `scores` and `failures` arrays; both batch functions now return it. `BatchScoresResult` implements `__len__`, `__getitem__`, `__array__`, `.shape`, `.dtype`, and `.n_failures` so callers using the result like an ndarray (`np.isnan(result)`, `result[mask]`, `result.shape`) keep working. `__array__` accepts NumPy 2's `copy: bool | None` keyword and mirrors plain-ndarray semantics: `np.asarray(result, copy=False)` returns the underlying buffer when no cast is needed and raises `ValueError` when a dtype cast would require a copy (silently allocating in that case would mask caller bugs). `border.py:243` Dijkstra-failure path: review-response changed it from `warnings.warn(...) + return np.nan` to `raise RuntimeError(...)`. The single-neuron `border_score` now surfaces the failure loudly; `batch_border_scores` catches it, increments the failure counter, and continues — preserving the per-neuron summary semantics while ending the silent-NaN substitution that hid Dijkstra failures from single-neuron callers. `batch_grid_scores` review-response: pre-checks legitimate-NaN inputs (constant firing, all-NaN rate map) before calling `spatial_autocorrelation` so they emit NaN with `failures=False` and no warning; the catch-all now flags only true computation exceptions, restoring the contract that the failures mask cleanly distinguishes "NaN by construction" from "NaN because we caught an exception". `SpatialRatesResult.classify()` and `.to_dataframe()` updated to read `.scores` from the wrapper. 8 total tests cover the M1 1.5 surface (3 from initial + 2 from the Dijkstra-raise review-response + 3 from the legitimate-NaN / NumPy-2 review-response)._
