...
- **Polar envs.** Add an `is_polar: bool` property and a `coordinate_kind: Literal["cartesian", "polar"]` attribute on `Environment`. Make Cartesian-only methods (`distance_to`, `distance_between`, `bin_at` on (x, y) input, `contains`, `apply_transform`) raise a clear `ValueError` for polar envs. `plot_field` switches axis labels to `Distance` / `Angle (rad)` and skips `set_aspect("equal")` rather than raising — this lets `EgocentricRateResult.plot()` continue to render polar firing fields. Update `from_polar_egocentric` to set the flag.
