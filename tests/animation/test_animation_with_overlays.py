...
class TestNapariBackendIntegration:
    ...
    def test_napari_with_head_direction_overlay(
        ...
        # Verify tracks layer added for head direction (uses Tracks layer)
        tracks_calls = [
            call
    ...
    def test_napari_with_all_overlays(
        ...
        )  # Skeleton only (head direction now uses Tracks)
        # Verify head direction creates Tracks layer (not Vectors or Points)
        tracks_calls = [
            call
            for call in mock_viewer.add_tracks.call_args_list
            if call[1].get("name", "").startswith("Head Direction")
...
class TestMixedOverlayTypes:
    ...
    def test_all_three_overlay_types_together(
        ...
        # Skeleton uses vectors layer (head direction now uses Tracks)
