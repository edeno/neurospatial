...
class TestComputePlaybackFps:
    ...
    def test_normal_case_30hz_realtime(self):
        ...
        # Since MAX_PLAYBACK_FPS=60, 30 Hz data is NOT capped
        frame_times = np.linspace(0, 10, 301)
        fps, actual_speed = _compute_playback_fps(frame_times, speed=1.0)

        assert fps == 30  # Not capped since 30 < 60
        assert actual_speed == pytest.approx(1.0, rel=0.01)  # Full speed

    def test_capping_500hz_data(self):
        """Test that 500 Hz data at speed=1.0 is capped to 60 fps."""
        from neurospatial.animation.core import MAX_PLAYBACK_FPS, _compute_playback_fps

        # 500 Hz data, 1 second (501 frames)
        frame_times = np.linspace(0, 1, 501)
        fps, actual_speed = _compute_playback_fps(frame_times, speed=1.0)

        assert fps == MAX_PLAYBACK_FPS  # Capped to MAX_PLAYBACK_FPS (60)
        assert actual_speed == pytest.approx(60 / 500, rel=0.01)  # 60/500 = 0.12
    ...
    def test_single_frame_edge_case(self):
        ...
        assert fps == MAX_PLAYBACK_FPS  # Returns max_fps (60)
    ...
    def test_fast_forward_2x(self):
        ...
        # 30 Hz * 2x = 60 fps requested, equals MAX_PLAYBACK_FPS=60
        assert fps == MAX_PLAYBACK_FPS  # Capped to MAX_PLAYBACK_FPS (60)
        assert actual_speed == pytest.approx(2.0, rel=0.01)  # Full 2x speed achieved
...
class TestAnimateFieldsSpeedBasedPlayback:
    ...
    def test_max_playback_fps_parameter_default_is_60(self):
        ...
        from neurospatial.animation.core import MAX_PLAYBACK_FPS, animate_fields
        ...
        with patch("neurospatial.animation.backends.html_backend.render_html") as mock:
            ...
            # Call without max_playback_fps - should use default MAX_PLAYBACK_FPS (60)
            animate_fields(
                env,
                fields,
                backend="html",
            ...
            # Check that fps is capped at MAX_PLAYBACK_FPS (60)
            call_kwargs = mock.call_args[1]
            assert call_kwargs.get("fps") <= MAX_PLAYBACK_FPS
    ...
    def test_fps_kwarg_is_ignored(self):
        ...
        with patch("neurospatial.animation.backends.html_backend.render_html") as mock:
            ...
            call_kwargs = mock.call_args[1]
            # fps should be 30 (computed from frame_times), not 100
            # Since MAX_PLAYBACK_FPS=60, 30 Hz data is NOT capped (30 < 60)
    ...
    def test_no_warning_when_speed_not_capped(self):
        ...
        # Create 20 Hz data - at speed=1.0 would require 20 fps (within MAX=60 limit)
