...
# ==============================================================================
# Task 3.11: Verify metrics match existing HeadDirectionMetrics class
# ==============================================================================


class TestDirectionalRateResultMatchesHeadDirectionMetrics:
    """Verify DirectionalRateResult metrics match legacy HeadDirectionMetrics.

    These tests ensure backwards compatibility by verifying that the new
    DirectionalRateResult convenience methods produce equivalent results
    to the legacy HeadDirectionMetrics dataclass from head_direction.py.
    """

    @pytest.fixture
    def sharply_tuned_data(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Create data with a sharply tuned neuron (peak at π/2)."""
        np.random.seed(42)
        n_bins = 60
        bin_centers = np.linspace(0, 2 * np.pi, n_bins, endpoint=False)
        # Create von Mises-like tuning curve with peak at π/2
        preferred_dir = np.pi / 2
        kappa = 5.0  # Concentration parameter (higher = sharper)
        firing_rate = 10.0 * np.exp(kappa * (np.cos(bin_centers - preferred_dir) - 1))
        occupancy = np.ones(n_bins) * 0.5  # Uniform occupancy
        return bin_centers, firing_rate, occupancy

    @pytest.fixture
    def uniform_firing_data(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Create data with uniform (non-directional) firing."""
        np.random.seed(42)
        n_bins = 60
        bin_centers = np.linspace(0, 2 * np.pi, n_bins, endpoint=False)
        # Uniform firing with small noise
        firing_rate = np.ones(n_bins) * 5.0 + np.random.randn(n_bins) * 0.1
        occupancy = np.ones(n_bins) * 0.5
        return bin_centers, firing_rate, occupancy

    def test_preferred_direction_matches(
        self, sharply_tuned_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.preferred_direction matches HeadDirectionMetrics."""
        from neurospatial.encoding.directional import DirectionalRateResult
        from neurospatial.encoding.head_direction import head_direction_metrics

        bin_centers, firing_rate, occupancy = sharply_tuned_data
        ...
        new_pref_dir = result.preferred_direction()

        # Legacy API
        legacy_metrics = head_direction_metrics(bin_centers, firing_rate)
        legacy_pref_dir = legacy_metrics.preferred_direction

        # Both should be close to π/2 and match each other
        # Note: preferred_direction returns [-π, π], legacy uses [0, 2π]
        # Normalize both to [-π, π] for comparison
        legacy_normalized = legacy_pref_dir
        if legacy_normalized > np.pi:
            legacy_normalized -= 2 * np.pi

        np.testing.assert_allclose(new_pref_dir, legacy_normalized, atol=0.01)

    def test_mean_vector_length_matches(
        self, sharply_tuned_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.mean_vector_length matches HeadDirectionMetrics."""
        from neurospatial.encoding.directional import DirectionalRateResult
        from neurospatial.encoding.head_direction import head_direction_metrics
        ...
        new_mvl = result.mean_vector_length()

        # Legacy API
        legacy_metrics = head_direction_metrics(bin_centers, firing_rate)
        legacy_mvl = legacy_metrics.mean_vector_length

        np.testing.assert_allclose(new_mvl, legacy_mvl, rtol=1e-10)

    def test_peak_firing_rate_matches(
        self, sharply_tuned_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.peak_firing_rate matches HeadDirectionMetrics."""
        from neurospatial.encoding.directional import DirectionalRateResult
        from neurospatial.encoding.head_direction import head_direction_metrics
        ...
        new_peak = result.peak_firing_rate()

        # Legacy API
        legacy_metrics = head_direction_metrics(bin_centers, firing_rate)
        legacy_peak = legacy_metrics.peak_firing_rate

        np.testing.assert_allclose(new_peak, legacy_peak, rtol=1e-10)

    def test_rayleigh_pvalue_matches(
        self, sharply_tuned_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.rayleigh_pvalue matches HeadDirectionMetrics."""
        from neurospatial.encoding.directional import DirectionalRateResult
        from neurospatial.encoding.head_direction import head_direction_metrics
        ...
        new_pval = result.rayleigh_pvalue()

        # Legacy API
        legacy_metrics = head_direction_metrics(bin_centers, firing_rate)
        legacy_pval = legacy_metrics.rayleigh_pval

        np.testing.assert_allclose(new_pval, legacy_pval, rtol=1e-10)

    def test_is_hd_cell_matches(
        self, sharply_tuned_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.is_hd_cell matches HeadDirectionMetrics classification."""
        from neurospatial.encoding.directional import DirectionalRateResult
        from neurospatial.encoding.head_direction import head_direction_metrics
        ...
        new_is_hd = result.is_hd_cell()

        # Legacy API
        legacy_metrics = head_direction_metrics(bin_centers, firing_rate)
        legacy_is_hd = legacy_metrics.is_hd_cell

        assert new_is_hd == legacy_is_hd

    def test_is_hd_cell_matches_uniform(
        self, uniform_firing_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.is_hd_cell matches for non-HD cells."""
        from neurospatial.encoding.directional import DirectionalRateResult
        from neurospatial.encoding.head_direction import head_direction_metrics

        bin_centers, firing_rate, occupancy = uniform_firing_data
        ...
        assert new_is_hd == legacy_is_hd
        assert new_is_hd is False  # Both should say not an HD cell

    def test_tuning_width_same_order_of_magnitude(
        self, sharply_tuned_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.tuning_width is similar to HeadDirectionMetrics.

        Note: The algorithms may differ slightly, so we check they're in the
        same ballpark rather than exact equality.
        """
        from neurospatial.encoding.directional import DirectionalRateResult
        from neurospatial.encoding.head_direction import head_direction_metrics
        ...
        new_width = result.tuning_width()

        # Legacy API
        legacy_metrics = head_direction_metrics(bin_centers, firing_rate)
        legacy_width = legacy_metrics.tuning_width

        # Both should be in the same ballpark (within 50%)
        # The algorithms may differ slightly in how they compute HWHM
        assert abs(new_width - legacy_width) / legacy_width < 0.5

    def test_preferred_direction_deg_matches(
        self, sharply_tuned_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.preferred_direction_deg matches HeadDirectionMetrics."""
        from neurospatial.encoding.directional import DirectionalRateResult
        from neurospatial.encoding.head_direction import head_direction_metrics
        ...
        new_pref_deg = result.preferred_direction_deg()

        # Legacy API
        legacy_metrics = head_direction_metrics(bin_centers, firing_rate)
        legacy_pref_deg = legacy_metrics.preferred_direction_deg

        # Normalize to same range for comparison
        # New API returns [-180, 180], legacy returns [0, 360]
        legacy_normalized = legacy_pref_deg
        if legacy_normalized > 180:
            legacy_normalized -= 360

        np.testing.assert_allclose(new_pref_deg, legacy_normalized, atol=1.0)

    def test_interpretation_contains_key_info_for_hd_cell(
        self, sharply_tuned_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.interpretation contains expected info for HD cell."""
        from neurospatial.encoding.directional import DirectionalRateResult
        ...
        assert "Preferred direction" in interp
        assert "Mean vector length" in interp
        assert "Peak firing rate" in interp

    def test_interpretation_contains_key_info_for_non_hd_cell(
        self, uniform_firing_data: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        """DirectionalRateResult.interpretation contains expected info for non-HD cell."""
        from neurospatial.encoding.directional import DirectionalRateResult
